...
{
    DXVDecContext *ctx = avctx->priv_data;
    ...
    AV_WL32(ctx->base.tex_data, bytestream2_get_le32(gbc));
    AV_WL32(ctx->base.tex_data + 4, bytestream2_get_le32(gbc));

    /* Process input until the whole texture has been filled */
    while (pos + 2 <= ctx->base.tex_size / 4) {
        CHECKPOINT(2);

        /* Copy two elements from a previous offset or from the input buffer */
        if (op) {
            prev = AV_RL32(ctx->base.tex_data + 4 * (pos - idx));
            AV_WL32(ctx->base.tex_data + 4 * pos, prev);
            pos++;
...
static int dxv_decompress_cgo(DXVDecContext *ctx, GetByteContext *gb,
...
static int dxv_decompress_cocg(DXVDecContext *ctx, GetByteContext *gb,
                               uint8_t *tex_data, int tex_size,
...
static int dxv_decompress_yo(DXVDecContext *ctx, GetByteContext *gb,
                             uint8_t *tex_data, int tex_size,
...
static int dxv_decompress_ycg6(AVCodecContext *avctx)
{
    ...
    ret = dxv_decompress_yo(ctx, gb, ctx->base.tex_data, ctx->base.tex_size,
                            ctx->op_data[0], ctx->op_size[0]);
    if (ret < 0)
        return ret;

    return dxv_decompress_cocg(ctx, gb, ctx->base.ctex_data, ctx->base.ctex_size,
...
static int dxv_decompress_yg10(AVCodecContext *avctx)
{
    ...
    ret = dxv_decompress_cocg(ctx, gb, ctx->base.tex_data, ctx->base.tex_size,
...
static int dxv_decompress_dxt5(AVCodecContext *avctx)
{
    ...
    AV_WL32(ctx->base.tex_data +  0, bytestream2_get_le32(gbc));
    AV_WL32(ctx->base.tex_data +  4, bytestream2_get_le32(gbc));
    AV_WL32(ctx->base.tex_data +  8, bytestream2_get_le32(gbc));
    AV_WL32(ctx->base.tex_data + 12, bytestream2_get_le32(gbc));
    ...
    while (pos + 2 <= ctx->base.tex_size / 4) {
        if (run) {
            ...
            prev = AV_RL32(ctx->base.tex_data + 4 * (pos - 4));
            AV_WL32(ctx->base.tex_data + 4 * pos, prev);
            pos++;
            prev = AV_RL32(ctx->base.tex_data + 4 * (pos - 4));
            AV_WL32(ctx->base.tex_data + 4 * pos, prev);
            ...
            case 0:
                ...
                while (check && pos + 4 <= ctx->base.tex_size / 4) {
                    prev = AV_RL32(ctx->base.tex_data + 4 * (pos - 4));
                    AV_WL32(ctx->base.tex_data + 4 * pos, prev);
                    pos++;
            ...
            case 2:
                ...
                if (idx > pos || (unsigned int)(pos - idx) + 2 > ctx->base.tex_size / 4)
                    return AVERROR_INVALIDDATA;
                prev = AV_RL32(ctx->base.tex_data + 4 * (pos - idx));
                AV_WL32(ctx->base.tex_data + 4 * pos, prev);
                pos++;
        ...
        if (op) {
            ...
            if (op && (idx > pos || (unsigned int)(pos - idx) + 2 > ctx->base.tex_size / 4))
                return AVERROR_INVALIDDATA;
            if (op)
                prev = AV_RL32(ctx->base.tex_data + 4 * (pos - idx));
            else
...
{
    DXVDecContext *ctx = avctx->priv_data;
    return ff_lzf_uncompress(&ctx->gbc, &ctx->base.tex_data, &ctx->base.tex_size);
}
...
static int dxv_decompress_raw(AVCodecContext *avctx)
{
    ...
    if (bytestream2_get_bytes_left(gbc) < ctx->base.tex_size)
        return AVERROR_INVALIDDATA;

    bytestream2_get_buffer(gbc, ctx->base.tex_data, ctx->base.tex_size);
    return 0;
}
                      ...
                      int *got_frame, AVPacket *avpkt)
{
    ...
    ctx->base.tex_fmt = bytestream2_get_le32(gbc);
    switch (ctx->base.tex_fmt) {
    case DXV_FMT_DXT1:
        ctx->base.process_outer_tex = dxv_decompress_dxt1;
        msgcomp = "DXTR1";
        msgtext = "DXT1";
        break;
    case DXV_FMT_DXT5:
        ctx->base.process_outer_tex = dxv_decompress_dxt5;
        msgcomp = "DXTR5";
        msgtext = "DXT5";
        break;
    case DXV_FMT_YCG6:
        ctx->base.process_outer_tex = dxv_decompress_ycg6;
        msgcomp = "YOCOCG6";
        msgtext = "YCG6";
        ...
        break;
    case DXV_FMT_YG10:
        ctx->base.process_outer_tex = dxv_decompress_yg10;
        msgcomp = "YAOCOCG10";
        msgtext = "YG10";
    ...
    default:
        ...
        size = ctx->base.tex_fmt & 0x00FFFFFF;
        old_type = ctx->base.tex_fmt >> 24;
        version_major = (old_type & 0x0F) - 1;

        if (old_type & 0x80) {
            msgcomp = "RAW";
            ctx->base.process_outer_tex = dxv_decompress_raw;
        } else {
            msgcomp = "LZF";
            ctx->base.process_outer_tex = dxv_decompress_lzf;
        }

        if (old_type & 0x40) {
            msgtext = "DXT5";
            ctx->base.tex_fmt = DXV_FMT_DXT5;
        } else if (old_type & 0x20 || version_major == 1) {
            msgtext = "DXT1";
            ctx->base.tex_fmt = DXV_FMT_DXT1;
        } else {
    ...
    ret = ff_dxv_prepare_ctx(avctx, &ctx->base);
    if (ret < 0)
        return ret;
    ...
    if (ctx->base.ctexdspctx.tex_funct) {
    ...
    ret = ctx->base.process_outer_tex(avctx);
    if (ret < 0)
        return ret;
    ...
    ret = ff_dxv_decompress_inner_tex(avctx, &ctx->base, frame);
    if (ret < 0)
        return ret;
...
static int dxv_init(AVCodecContext *avctx)
{
    ...
    ff_texturedsp_init(&ctx->base.texdsp);
...
const FFCodec ff_dxv_decoder = {
    ...
    .close          = dxv_close,
    .priv_data_size = sizeof(DXVDecContext),
